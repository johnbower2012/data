...
arma::mat kernel_derivative_function(arma::mat kernel_func(arma::mat, arma::mat, arma::vec), arma::mat A, arma::mat B, arma::vec hyperp_vec, int selection, double precision){
	int a = A.n_rows,
		b = B.n_cols;
	...
	arma::mat 	kernel_plus = arma::zeros<arma::mat>(a,b),
				kernel_minus = arma::zeros<arma::mat>(a,b);
	...
	kernel_plus = kernel_func(A,B,temp_plus);
	kernel_minus = kernel_func(A,B,temp_minus);

	return (kernel_plus - kernel_minus)/(2.0*precision);
}
...
arma::mat kernel_periodic_function(arma::mat A, arma::mat B, arma::vec hyperp_vec){
	...
	theta1 = hyperp_vec(0)*hyperp_vec(0);
	theta2 = 2.0/hyperp_vec(1)/hyperp_vec(1);
	...
	for(i=0;i<ai;i++){
		for(j=0;j<bi;j++){
			...
			kernel_matrix(i,j) = theta1*exp(-sum);
		}
	}
...
arma::mat kernel_co2_function(arma::mat A, arma::mat B, arma::vec hyperp_vec){
	...
	kernel_matrix = kernel_square_exp_function(A,B,hyperp_1);
	kernel_matrix += kernel_periodic_decayaway_function(A,B,hyperp_2);
	kernel_matrix += kernel_rational_quadratic_function(A,B,hyperp_3);
	kernel_matrix += kernel_square_exp_noise_function(A,B,hyperp_4);
...
**********************************************/
long double log_likelihood_function(arma::mat Y_mat, arma::mat X_mat, arma::mat kernel_func(arma::mat, arma::mat, arma::vec), arma::vec hyperp_vec){
	int i,
		train = X_mat.n_rows;
	...
	arma::mat 	kernel_mat = arma::zeros<arma::mat> (train,train),
				kernel_inv_mat = arma::zeros<arma::mat> (train,train),
	...
	value_mat = -0.5*Y_mat.t()*kernel_inv_mat*Y_mat;
	arma::eig_sym(eigval,kernel_mat);
	value = 1.0;
	for(i=0;i<train;i++){
		value *= eigval(i);
	}
...
arma::vec log_likelihood_range_function(arma::mat Y_mat, arma::mat X_mat, int resolution, arma::mat kernel_func(arma::mat, arma::mat, arma::vec), arma::vec hyperp_vec, arma::mat hyperp_range_vec, int selection){
	int i, j,
		train = X_mat.n_rows,
	...
	for(i=0;i<resolution;i++){
		...
		kernel_mat = kernel_func(X_mat, X_mat, hyperp_temp_vec);
		kernel_mat += I*1e-8;
		kernel_inv_mat = kernel_mat.i();

		value_mat = -0.5*Y_mat.t()*kernel_inv_mat*Y_mat;
		log_likelihood_vec(i) = value_mat(0,0);
		log_likelihood_vec(i) += -0.5*log(arma::det(kernel_mat));
	}

	return log_likelihood_vec;
}
arma::vec log_likelihood_spec_function(arma::mat Y_mat, arma::mat X_mat, arma::mat kernel_func(arma::mat, arma::mat, arma::vec), arma::vec hyperp_vec, arma::vec hyperp_value_vec, int selection){
	int i, j,
		train = X_mat.n_rows,
		hyperp = hyperp_vec.n_elem,
...
arma::vec log_likelihood_derivative(arma::mat Y_mat, arma::mat X_mat, int resolution, arma::mat kernel_func(arma::mat, arma::mat, arma::vec), arma::vec hyperp_vec, arma::mat hyperp_range_vec, int selection, double precision){
	int i,
		train = X_mat.n_rows,
		observables = Y_mat.n_cols,
		hyperp = hyperp_vec.n_elem;
	...
	arma::mat 	kernel_mat = arma::zeros<arma::mat> (train,train),
				kernel_inv_mat = kernel_mat,
				kernel_derivative_mat = kernel_mat,
				I = arma::eye<arma::mat>(train,train),
	...
	for(i=0;i<resolution;i++){
		...
		alpha = kernel_inv_mat*Y_mat;

		kernel_derivative_mat = kernel_derivative_function(kernel_func, X_mat, X_mat, hyperp_vec, selection, precision);

		value = 0.5*arma::trace((alpha*alpha.t() - kernel_inv_mat)*kernel_derivative_mat);
